...
StFttPointMaker::StFttClusterPointMaker( const char* name )
: StMaker( name ),
...
void StFttClusterPointMaker::MakeLocalPoints(UChar_t Rob)
{
    ...
    for (int iClu_DH = 0; iClu_DH < clustersPerRob[(UChar_t)Rob][kFttDiagonalH].size(); iClu_DH++)
    {
        ...
        if (clu_DH->row() <= 2)
        {
            LOG_ERROR << "Wrong cluster row information for cluster at Rob " << Rob << " direction : " << Direction_name[kFttDiagonalH] <<  "!!!!!!" << endm;
            continue;
        }
    ...
    for (int iClu_DV = 0; iClu_DV < clustersPerRob[(UChar_t)Rob][kFttDiagonalV].size(); iClu_DV++)
    {
        ...
        if (clu_DV->row() <= 2)
        {
            LOG_ERROR << "Wrong cluster row information for cluster at Rob " << Rob << " direction : " << Direction_name[kFttDiagonalV] <<  "!!!!!!" << endm;
            continue;
        }
